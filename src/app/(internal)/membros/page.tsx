...
interface Membro { id: string; nome: string; email: string; tipo: string }
type TipoKey = 'admin' | 'lider' | 'membro'

export default function Membros() {
  const [membros, setMembros] = useState<Membro[]>([])
  const [nome, setNome] = useState('')
  const [email, setEmail] = useState('')
  const [tipo, setTipo] = useState<TipoKey>('membro')
...
async function handleSalvar() {
  ...
  const tipoConfig: Record<TipoKey, { cor: string; texto: string; label: string }> = {
  ...
  return (
    <div className="p-6 max-w-3xl">
      ...
      <div className="rounded-2xl p-6 mb-6" style={{background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.08)"}}>
        ...
        <select value={tipo} onChange={e => setTipo(e.target.value as TipoKey)} className="w-full px-4 py-3 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 mb-4" style={selectStyle}>
        ...
        {membros.length === 0 ? (
          ...
          <div key={m.id} className="flex justify-between items-center px-6 py-4" style={{borderBottom: "1px solid rgba(255,255,255,0.05)"}}>
            ...
            <div className="flex items-center gap-2">
              <span className="text-xs px-3 py-1 rounded-full font-medium" style={{background: tipoConfig[m.tipo as TipoKey]?.cor, color: tipoConfig[m.tipo as TipoKey]?.texto}}>{tipoConfig[m.tipo as TipoKey]?.label}</span>
